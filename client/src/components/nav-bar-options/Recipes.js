...
export default function Recipes(props) {
  const [date, setDate] = useState(props.location.state?.day || new Date().toISOString().split("T")[0]);
  const [startTime, setStartTime] = useState(props.location.state?.element.startTime ||
                                        new Date().toLocaleTimeString("en-US",
                                         { hour12: false }).substring(0, 5));
  ...
  const getUserData = () => {
    axios
      .get(`/api/days/user/${props.user._id}`)
      .then((res) => {
        if (res.data === null) {
          setDayEmpty(true);
        } else {
          setFoods(flattenDays(res.data));
        }
      })
  ...
  const handleClick = (event) => {
    ...
    const clickedRecipe = foods.find((food) => food._id === key);
    const payload = {
      user: user,
      date: date,
      food: { ...clickedRecipe, startTime: startTime },
    };
    axios
      .post(
        `/api/ingredients/user/${props.user._id}/day/${date}`,
        payload
      )
      .then(() => {
