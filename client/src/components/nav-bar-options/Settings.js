...
export default class Settings extends React.Component {
  ...
  handleSubmit=event=>{
    ...
    axios.put(`/api/users/${this.props.user._id}`,{data:[newEmail,newPassword]})
      .then(res=>{
        console.log(res)
      })
    ...
    return (
      <div>
        ...
        <form onSubmit={this.handleSubmit} className='flex flex-column items-center' action="POST">
          ...
          <input className="mb2" id="new-password" name="newPassword" onChange={this.handleChange} value={this.state.newPassword} type="text"/>
